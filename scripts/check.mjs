...
for (const type of ['WebPage', 'WebSite', 'Organization', 'Article', 'FAQPage', 'HowTo', 'BreadcrumbList']) assert(byType[type], type);
...
for (const type of ['WebPage', 'Article']) {
  assert.equal(byType[type].datePublished, dates[0]);
  assert.equal(byType[type].dateModified, dates[1]);
  assert.equal(byType[type].url, base);
}
...
for (const name of ['index.html', 'styles.css', 'script.js', '_headers', 'favicon.svg', 'og-image.svg', 'og-image.png']) assert(existsSync(path.join(root, name)), `Missing asset: ${name}`);
const png = readFileSync(path.join(root, 'og-image.png'));
assert.equal(png.readUInt32BE(16), 1200);
assert.equal(png.readUInt32BE(20), 630);
console.log('PASS: domain, PR disclosure, five offers, four real players, seven schemas, FAQ/HowTo, Google tags, CSP hash, anchors and social image.');
